...
CHT_CHUNK_TYPE_IMPLEMENTATION((RetryChunk));
void RetryChunk::writeToBuffer(char * dataBuffer, size_t const bufferSize) const
{
	if(bufferSize != this->getSize())
	{
		throw std::runtime_error("Wrong buffer size to RetryChunk::writeToBuffer.");
	}
	else
	{
		...
		offset += sizeof(unsigned int);
		memcpy(&dataBuffer[offset], &this->round, sizeof(unsigned int));
		offset += sizeof(unsigned int);
...
void RetryChunk::assignFromBuffer(char const * dataBuffer, size_t const bufferSize)
{
	if (bufferSize != this->getSize())
	{
		throw std::runtime_error("Wrong buffer size to RetryChunk::assign_from_buffer.");
	}
	else
	{
		//Copy stuff, one at a time
		...
		offset += sizeof(unsigned int);
		memcpy((void *)&this->round, &dataBuffer[offset], sizeof(unsigned int));
		offset += sizeof(unsigned int);
...
size_t RetryChunk::getSize() const
{
	size_t retSize = 0;
...
size_t RetryChunk::memoryUsage() const
{
	return this->getSize();
}
