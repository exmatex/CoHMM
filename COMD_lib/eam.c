...
int eamForce(SimFlat* s)
{
   ...
   // zero sress/energy flux
   memset(s->defInfo->stress,  0, 10*sizeof(real_t));
   memset(s->defInfo->flux,  0, 3*sizeof(real_t));
   for (int k=0; k<3; k++)
   {
      s->defInfo->flux[k] =0.0;
   }
   ...
   for (int iBox=0; iBox<s->boxes->nLocalBoxes; iBox++)
   {
      ...
      for (int jTmp=0; jTmp<nNbrBoxes; jTmp++)
      {
	   ...
	   /// kin energy computation for energy flux
	   real_t invMassI = 1.0/s->species[s->atoms->iSpecies[iOff]].mass;

	   real_t  eKinOverMass = 0.5*(s->atoms->p[iOff][0]*s->atoms->p[iOff][0] +
				       s->atoms->p[iOff][1]*s->atoms->p[iOff][1] +
				       s->atoms->p[iOff][2]*s->atoms->p[iOff][2] )*invMassI*invMassI;
	   
	   s->defInfo->flux[0] += eKinOverMass*s->atoms->p[iOff][0];
	   s->defInfo->flux[1] += eKinOverMass*s->atoms->p[iOff][1];
	   s->defInfo->flux[2] += eKinOverMass*s->atoms->p[iOff][2];
            ...
            for (int jOff=MAXATOMS*jBox,ij=0; ij<nJBox; ij++,jOff++)
            {
	           ...
	           //// energy flux (phi accumulation)
	           real_t invMassJ = 1.0/s->species[s->atoms->iSpecies[jOff]].mass;

               for (int k=0; k<3; k++)
               {
		         s->defInfo->flux[k] += phiTmp*(invMassI*s->atoms->p[iOff][k]);
	           }
      ...
      for (int jTmp=0; jTmp<nNbrBoxes; jTmp++)
      {
	   ...
	   real_t invMassI = 1.0/s->species[s->atoms->iSpecies[iOff]].mass;
            ...
            { 
	           ...
	           for (int k=0; k<3; k++){
		         s->defInfo->flux[k] += \
		         0.25*(invMassI*s->atoms->p[iOff][k] + invMassJ*s->atoms->p[jOff][k]) *
		         ( (pot->dfEmbed[iOff]+pot->dfEmbed[jOff])*dRho*(dr[k]/r) ) *
		         (s->atoms->p[iOff][k] - s->atoms->p[jOff][k]);
	           }
            } // loop over atoms in jBox
         } // loop over atoms in iBox
      } // loop over neighbor boxes
...
/////stress normalization
   for (int m = 0;m<9;m++) 
   {
      s->defInfo->stress[m] /= s->defInfo->globalVolume;
   }
/////flux normalization
   for (int m=0 ; m<3; m++) 
   {
      s->defInfo->flux[m] /= s->defInfo->globalVolume;
   }
...
void eamDestroy(BasePotential** pPot)
{
   ...
   comdFree(pot->dfEmbed);
   comdFree(pot->rhobar);
   comdFree(pot->forceExchangeData);
   comdFree(pot);
   *pPot = NULL;
...
void eamDestroyFinal(BasePotential** pPot)
{
   if ( ! pPot ) return;
   EamPotential* pot = *(EamPotential**)pPot;
   if ( ! pot ) return;
...
void eamReadSetfl(EamPotential* pot, const char* dir, const char* potName)
{
   ...
   FILE* potFile = fmemopen(fakePotFile, strlen(fakePotFile), "r");
   if (potFile == NULL)
      fileNotFound("eamReadSetfl", tmp);

   //printf("fake pot here\n");
   
   // read the first 3 lines (comments)
   char *cret;
   cret=fgets(tmp, sizeof(tmp), potFile);
   cret=fgets(tmp, sizeof(tmp), potFile);
   cret=fgets(tmp, sizeof(tmp), potFile);
   ...
   double mass, lat;
   char latticeType[8];
   ret=sscanf(tmp, "%d %le %le %s", &nAtomic, &mass, &lat, latticeType);
   ...
   for (int ii=0; ii<nRho; ++ii){
      ret=fscanf(potFile, FMT1, buf+ii);
   }
...
void eamReadFuncfl(EamPotential* pot, const char* dir, const char* potName)
{
   ...
   // line 1
   char *cret=fgets(tmp, sizeof(tmp), potFile);
   char name[3];
   sscanf(tmp, "%s", name);
