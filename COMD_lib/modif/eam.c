...
////stress phi force term (why 1.0, not 0.5?) 
               for (int i=0; i<3; i++)
               {
                  for (int j=0; j<3; j++)
                  {
	       ...
	       real_t invMassJ = 1.0/s->species[s->atoms->iSpecies[jOff]].mass;
               ...
               for (int k=0; k<3; k++)
               {
		 s->defInfo->flux[k] = 0.5*phiTmp*(invMassI*s->atoms->p[iOff][k] + 
						   invMassJ*s->atoms->p[jOff][k]);
	       }
...
/////stress, rho force term why 1.0 instead of 0.5?
               for (int i=0; i<3; i++)
               {
                  for (int j=0; j<3; j++)
                  {
