...
int calcEnergyFluxEam(SimFlat* s){
...
//    //energy flux computation added here
//    for (int m = 0;m<3;m++) 
//    {
//       s->defInfo->flux[m] = 0.0;
//    }
...
//                int iSpecies = s->atoms->iSpecies[iOff];
//                real_t invMass = 1.0/s->species[iSpecies].mass;
// /////flux, rho force term why 1.0 instead of 0.5?
//                real_t scalarpro = 0.0;
//                for (int i=0; i<3; i++)
//                {
//                  for (int k=0; k<3; k++){
//                    scalarpro += s->atoms->p[iOff][k]*invMass + s->atoms->p[jOff][k]*invMass*(pot->dfEmbed[iOff]+pot->dfEmbed[jOff])*dRho*(dr[i]/r);
//                  }
//                  s->defInfo->flux[i] += 0.25*scalarpro*dr[i];
//                }
//          //flux calcutation, Energy part
//          real_t localEKin = 0.0;
//          real_t localEPot = 0.0;
//          for (int i=0; i<3; i++){
//              localEKin += 0.5*s->atoms->p[iOff][i]*s->atoms->p[iOff][i]*invMass;
//              /////FIXME how do I get pair potential energy (table?)
//              localEPot += 0.5;
//              real_t localVel = 0.0;
//              s->defInfo->flux[i] += 0.5*localVel;
//          }
//          for (int i=0; i<3; i++){
//              s->defInfo->flux[i] += s->atoms->p[iOff][i]*invMass*localEKin*localEPot;
//          }
...
//    for (int m = 0;m<3;m++) 
//    {
//       s->defInfo->flux[m] = s->defInfo->flux[m]/s->defInfo->globalVolume;
//    }
