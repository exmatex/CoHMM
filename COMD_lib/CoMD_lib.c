...
///WARNING: DOES NOT SUPPORT MPI. POTENTIAL FOR REALLY BAD STUFF TO HAPPEN
...
CoMD_return CoMD_lib(CoMD_input *inputStruct)
{
   ...
   FILE *comdOut = fopen(fileName, "w");
   if (comdOut == NULL) {
       printf("Error writing datafile < %s >!\n", fileName);
       exit(0);
   }
   //Temp output file for part pos data
   char fileName2[255];
   char fileName3[255];
   sprintf(fileName3,"correlation%i_%i.dat", cmd.calls, cmd.rank);
#endif

   real_t averageStress[9] = {0.0}; 
   real_t averageEnergyFlux[3] = {0.0}; 
   real3 avPos[sim->atoms->nGlobal];
   for (int iOff = 0; iOff < sim->atoms->nGlobal; iOff++)
   {
     zeroReal3(avPos[iOff]);
   }
   ...
   real_t* g = NULL;
   real_t** stress = NULL;
   int length = ceil(nSteps/printRate);
   //initStressAutoCorr(&g, &stress, length);
   ...
   //printThings(sim, iStep, getElapsedTime(timestepTimer));
      //printThingsToFile(comdout, sim, iStep, getElapsedTime(timestepTimer), averageStress[0], avCounter);
      //number of steps used to perform the average calc of the stress -> if stress still flucts you need to either increase this value (nSteps/10) or the overall CoMd steps (nSteps) 
      //calcAveragePosition(sim, avPos, &posCounter);
      //sprintf(fileName2,"part%i_%i_%i.vtk", cmd.calls, cmd.rank, iStep);
      //writeVtk(fileName2, sim);
      //writeAverageVtk(fileName2, sim, avPos, posCounter);
      //calcStress(sim);
      //stressAutoCorrelation(sim, sim->defInfo->stress, g, stress, &calls);
      //printThingsToFile(comdOut, sim, iStep, getElapsedTime(timestepTimer), averageStress[0], avCounter);
      if(iStep > ceil(nSteps-(2*nSteps/10))){
        //printThingsToFile(comdOut, sim, iStep, getElapsedTime(timestepTimer), averageStress[0], avCounter);
        calcAverageTensor(averageStress, sim->defInfo->stress);
	      calcAverageEnergyFlux(averageEnergyFlux, sim->defInfo->flux);
        avCounter++;
      }
   ...
   //printStressAutoCorr(fileName3, g, &calls, printRate, sim->dt);
   free(stress);
   free(g);
   ...
   //printThingsToFile(comdOut, sim, iStep, getElapsedTime(timestepTimer), averageStress[0], avCounter);
   //fclose(comdOut);
   CoMD_return retVal = printAverageTensor(avCounter, averageStress);
   retVal = printAverageFlux(avCounter, averageEnergyFlux);
   //CoMD_return retVal = printTensor(iStep, sim->defInfo->stress);
...
#if 1
   //compute the initial energies based on the energy and moment   um density of the macro solver 
   real_t comVelocity[3] = {0.0,0.0,0.0};
   real_t mass = 0.0;
   computeMass(sim, &mass);
   //int myRank = getMyRank();

   comVelocity[0] = cmd.momDens[0] * sim->defInfo->globalVolume/mass;
   comVelocity[1] = cmd.momDens[1] * sim->defInfo->globalVolume/mass;
   comVelocity[2] = cmd.momDens[2] * sim->defInfo->globalVolume/mass;
 
   real_t eKineticCom = 0.5*mass*(comVelocity[0]*comVelocity[0] + comVelocity[1]*comVelocity[1] + comVelocity[2]*comVelocity[2]); 
   real_t eTotal = cmd.enDens * sim->defInfo->globalVolume;
   //printf("edens %g eTot %g vol %g\n", cmd.enDens, eTotal, sim->defInfo->globalVolume);
   real_t ePotential = 0.;
   addRealParallel(&sim->ePotential, &ePotential, 1);
   //printf("epot %g \n", ePotential);
   sim->eKinetic = eTotal - ePotential - eKineticCom;
   real_t temperature = (sim->eKinetic/sim->atoms->nGlobal)/kB_eV/1.5;
   //printf("temp %g \n", temperature);
   if(temperature < 0.0)
   {
      printf("ERROR Temperature negative!\n");
      printf("ERROR Temperature: %g !\n", temperature);
      exit(0);
   }
   setTemperature(sim, temperature);
   setVcm(sim, comVelocity);
...
void initSubsystems(void)
{
   ...
   averageStressOut = fopen("averageStressOut.txt","w");
