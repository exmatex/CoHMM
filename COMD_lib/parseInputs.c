...
Command parseInputStruct(CoMD_input * inputStruct)
{
	...
	cmd.defGrad[0] = inputStruct->defGrad[0];
	cmd.defGrad[1] = inputStruct->defGrad[1];
	cmd.defGrad[2] = inputStruct->defGrad[2];
	cmd.defGrad[3] = inputStruct->defGrad[3];
	cmd.enDens = inputStruct->enDens;
	cmd.momDens[0] = inputStruct->momDens[0];
	cmd.momDens[1] = inputStruct->momDens[1];
	cmd.momDens[2] = inputStruct->momDens[2];
	cmd.rank = inputStruct->rank;
	cmd.calls = inputStruct->calls;
