...
Atoms* initAtoms(LinkCell* boxes)
{
   ...
   atoms->a =        (real3*) comdMalloc(maxTotalAtoms*sizeof(real3));
...
/// Computes the mass of the system.
void computeMass(SimFlat* s, real_t* totalMass)
{
   real_t massLocal = 0.;
   real_t massSum = 0.;
   ...
   for (int iBox=0; iBox<s->boxes->nLocalBoxes; ++iBox)
   {
      ...
      {
         int iSpecies = s->atoms->iSpecies[iOff];
         massLocal += s->species[iSpecies].mass;
      }
   }

   startTimer(commReduceTimer);
   addRealParallel(&massLocal, &massSum, 1);
   stopTimer(commReduceTimer);
