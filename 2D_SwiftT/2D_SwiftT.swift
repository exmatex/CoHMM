...
main
{
	...
	if(argC != 4)
	{
		printf("turbine ./2D_SwiftT <dim_x> <dim_y> <nsteps> <redis_server>\n");
	}
	else
	{
		...
		wait(rInit)
		{
			...
			for(int t = 0; t < numSteps; t = j[t])
			{
				wait(t)
				{
					...
					printf("%d: Starting", t) =>
					shortCircuit = shortCircuitWrapper(bDims, t) =>
					if(shortCircuit)
					{
						printf("%d: Short Circuit Successful! On to the next!", t);
						j[t] = t+1;
						if(j[t] == numSteps)
						{
							doneRun = true;
