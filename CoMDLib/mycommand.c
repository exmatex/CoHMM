...
/// | \--temp        | -T          | 1.0           | Energy density (eV*Angstrom^-3)   
/// | \--delta       | -r          | 0             | initial delta (Angstroms)
/// | \--defGrad     | -g          | 1.0           | 1D deformation gradient
/// | \--stressSuffix| -s          |               | Suffix to append to file containing stress tensor
///
/// Notes: 
/// 
...
Command parseCommandLine(int argc, char** argv)
{
   ...
   addArg("defGrad",    'g', 1, 'd',  &(cmd.defGrad),      0,             "1D deformation gradient");
   addArg("stressSuffix", 's', 1, 's', cmd.stressSuffix, sizeof(cmd.stressSuffix) ,	"Suffix to append to file containing stress tensor");
   addArg("energy",     'E', 1, 'd',  &(cmd.energy),  0,             "energy density (eV*Angstrom^-3)");
...
void printCmdYaml(FILE* file, Command* cmd)
{
   ...
   fprintf(file,
           ...
           "  printRate: %d\n"
           "  Time step: %g fs\n"
	   "  Input energy density: %g energy unit ?\n"
