   ...
   if (pot->forceExchange == NULL)
     {
       ...
       pot->forceExchange = initForceHaloExchange(s->domain, s->boxes); //UNCOMMENT
       pot->forceExchangeData = comdMalloc(sizeof(ForceExchangeData)); //UNCOMMENT
       pot->forceExchangeData->dfEmbed = pot->dfEmbed;//UNCOMMENT
       pot->forceExchangeData->boxes = s->boxes;//UNCOMMENT

       //TATA=pot->dfEmbed;
       //TITI=pot->rhobar;
       //TOTO=pot->forceExchangeData;
...
void eamBcastPotential(EamPotential* pot)
{
   ...
   if (getMyRank() == 0)
   {
 	...
 	strncpy ( buf.name,pot->name , sizeof(buf.name) );	
   }
...
strncpy ( pot->name,buf.name , sizeof(pot->name) );
