...
static CoMD_return printStuff(int iStep, SimFlat *s, double stressAvg[9]);
...
{
	CoMD_return retVal;
	retVal.simIter = iStep;
	if(s->nSteps>500){
	  retVal.stressXX = stressAvg[0];
	  retVal.stressXY = stressAvg[1];
	  retVal.stressXZ = stressAvg[2];
	  retVal.stressYX = stressAvg[3];
	  retVal.stressYY = stressAvg[4];
	  retVal.stressYZ = stressAvg[5];
	  retVal.stressZX = stressAvg[6];
	  retVal.stressZY = stressAvg[7];
	  retVal.stressZZ = stressAvg[8];
	}else{
	  retVal.stressXX = s->defInfo->stress[0];
	  retVal.stressXY = s->defInfo->stress[1];
	  retVal.stressXZ = s->defInfo->stress[2];
	  retVal.stressYX = s->defInfo->stress[3];
	  retVal.stressYY = s->defInfo->stress[4];
	  retVal.stressYZ = s->defInfo->stress[5];
	  retVal.stressZX = s->defInfo->stress[6];
	  retVal.stressZY = s->defInfo->stress[7];
	  retVal.stressZZ = s->defInfo->stress[8];
	}
	retVal.time = iStep*s->dt;
	retVal.eTot = (s->ePotential+s->eKinetic) / s->atoms->nGlobal;
   	retVal.eKin = s->eKinetic / s->atoms->nGlobal;
	retVal.ePot = s->ePotential / s->atoms->nGlobal;
	retVal.temp  = (s->eKinetic / s->atoms->nGlobal) / (kB_eV * 1.5);
...
int main(int argc, char** argv)
{
   ...
   if(cmd.stressSuffix[0] != 0)
   {
		fclose(stressOut);
		char stressName[13+1024];
		strcpy(stressName, "stressOut");
		strcat(stressName, cmd.stressSuffix);
		strcat(stressName, ".txt");
		stressOut = fopen(stressName,"w");
   }
...
SpeciesData* initSpecies(BasePotential* pot)
{
   ...
   strncpy(species->name, pot->name,sizeof(species->name));
